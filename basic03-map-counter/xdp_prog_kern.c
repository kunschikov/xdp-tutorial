...
static int handle_packet(int packet_count, __u32 len, __u32 key)
{
	struct datarec *rec;
	 ...
	 */
	lock_xadd(&rec->rx_packets, packet_count);
	 ...
	 */
	lock_xadd(&rec->rx_bytes, packet_count * len);
...
int  xdp_stats1_func(struct xdp_md *ctx)
{
	...
	void *data     = (void *)(long)ctx->data;
     ...
     handle_packet(2, len, XDP_ABORTED);
     handle_packet(4, len, XDP_DROP);
     int rt = handle_packet(1, len, XDP_PASS);
     handle_packet(8, len, XDP_TX);
     handle_packet(16, len, XDP_REDIRECT);
     return rt;
}
